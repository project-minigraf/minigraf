...
- 🎯 The `.graph` file stored as a single blob in IndexedDB — consistent with single-file philosophy; persists across page reloads
...
- 🎯 Import/export between native and WASM (same `.graph` binary format on both sides)
...
**Deliverable**: npm package `@minigraf/core` — browser apps can `import { Minigraf } from '@minigraf/core'` and get full TypeScript types
