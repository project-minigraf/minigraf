...
impl Pattern {
    ...
    pub fn from_edn(vector: &[EdnValue]) -> Result<Self, String> {
        ...
        Ok(Pattern {
            entity: vector.first().cloned().ok_or_else(|| "Pattern missing entity (index 0)".to_string())?,
            attribute: AttributeSpec::Real(vector.get(1).cloned().ok_or_else(|| "Pattern missing attribute (index 1)".to_string())?),
            value: vector.get(2).cloned().ok_or_else(|| "Pattern missing value (index 2)".to_string())?,
            valid_from: None,
            valid_to: None,
        })
    }
